...
function printList() {
    console.log('\n');
    const star = new Array(80).join('*')
    console.log(chalk.green(star));
    const keyArr = [];
    Object.keys(templates).forEach(function (key) {
        if (templates[key].isEnabled) {
            keyArr.push(key);
        }
        var arr = new Array(30);
        console.log(chalk.green((key + arr.join(' ')).substr(0, 15) + '    |    ' +
            (templates[key].isEnabled?' OK ':'TODO')  + '    |    ' + templates[key].description));
    });
    console.log(chalk.green(star));
    console.log('\n');
    return keyArr;
